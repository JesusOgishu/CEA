...
class AsanaService
{
    ...
    public function __construct()
    {
        ...
        if (!$user || !$token) {
            throw new \Exception('El usuario no tiene un token de Asana activo.');
        }
    ...
    public function getUsersByWorkspace(string $workspaceGid): array
    {
        return $this->request('GET', "workspaces/{$workspaceGid}/users", [
            'opt_fields' => 'name,photo.email,photo.image_32x32,photo.image_128x128'
        ])['data'] ?? [];
    }
}
