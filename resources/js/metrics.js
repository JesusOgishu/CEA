document.addEventListener("DOMContentLoaded", () => {
    ...
    if (workspaceSelect) {
        ...
        async function loadOverview(ep) {
            ...
            const activeProjectsEl = document.querySelector('#metric-active-projects');

            if (totalTasksEl) totalTasksEl.textContent = (data.open_tasks ?? 0) + (data.completed_last_days ?? 0);
            if (completedTasksEl) completedTasksEl.textContent = data.completed_last_days ?? 0;
            if (overdueTasksEl) overdueTasksEl.textContent = data.overdue_tasks ?? 0;
            if (activeProjectsEl) activeProjectsEl.textContent = data.active_projects ?? 0;
        }
